    ...
    <div>
      {(() => {
        ...
        const FormCmp = function ({ item, refetch }) {
          ...
          let examDataComponentAttribute = 'exam_form';
          const { getModel, getProperty, getCustomModelAttribute } = B;
          const { customMocustomModelAttribute: customModelAttributeObj } =
            options;
          ...
          if (!isDev && customModelAttributeObj) {
            if (customModelAttributeObj.id) {
              ...
              if (customAttributeObj.propertyId) {
                const customPropertyObj = getProperty(
                  customAttributeObj.propertyId,
                );
                if (customPropertyObj.modelId) {
                  const customPropertyModelObj = getModel(
                    customPropertyObj.modelId,
                  );
                  const propertyName = `${camelToSnakeCase(
                    customPropertyModelObj.name.toLowerCase(),
                  )}_${customPropertyObj.name.toLowerCase()}`;
                  examDataComponentAttribute = `${propertyName}_form`;
                }
              }
            }
          }
